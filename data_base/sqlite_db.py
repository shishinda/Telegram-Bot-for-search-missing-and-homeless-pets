...
def sql_start():
    ...
    base.execute('CREATE TABLE IF NOT EXISTS applications_from_users(client_pets_name TEXT, client_pets_breed TEXT, client_pets_signs TEXT, client_pets_photo TEXT, client_name TEXT, client_phone INT)')
    base.commit()
...
async def sql_add_client(state):
    async with state.proxy() as client_data:
        cur.execute('INSERT INTO applications_from_users VALUES (?, ?, ?, ?, ?, ?)', tuple(client_data.values()))
        base.commit()
...
async def sql_read_client(message):
    for ret in cur.execute('SELECT * FROM applications_from_users').fetchall():
        await bot.send_photo(message.from_user.id, ret[3], f'Кличка животного:{ret[0]}\nПорода:{ret[1]}\nПриметы:{ret[2]}\nИмя хозяина:{ret[4]}\nТелефон хозяина:{ret[5]}')
...
async def sql_read_client_for_delete():
    return cur.execute('SELECT * FROM applications_from_users').fetchall()
...
async def sql_delete_client_data(data):
    cur.execute('DELETE FROM applications_from_users WHERE client_pets_name == ?', (data,))
    base.commit()